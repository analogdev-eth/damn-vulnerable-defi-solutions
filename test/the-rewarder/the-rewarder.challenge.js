...
describe("[Challenge] The rewarder", function () {
  ...
  it("Execution", async function () {
    ...
    const rewarderExploitCF = await ethers.getContractFactory(
      "RewarderExploit",
      player
    );
    const RewarderExploit = await rewarderExploitCF.deploy(
      this.flashLoanPool.address,
    ...
    // Advance time 5 days so we can steal rewards
    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
    await RewarderExploit.attack();
  });
